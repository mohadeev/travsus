...
export default function DotsContainer({ days, onDayClick, selectedDayIndex }: DotsContainerProps) {
  ...
  const dotsContainerRefs = useRef<(HTMLDivElement | null)[]>(Array(days.length).fill(null))
  const contentRefs = useRef<(HTMLDivElement | null)[]>(Array(days.length).fill(null))
  ...
  useEffect(() => {
    const calculateDotsForDay = (index: number) => {
      ...
      const dotsCount = Math.floor((containerHeight - spacing) / totalPerDot) + 1
        ...
        return (
          <div key={index} className="flex flex-row items-stretch gap-2">
            ...
            <div className="flex flex-col items-center">
              <Button
                ...
                className={`mb-0 flex h-10 w-10 items-center justify-center rounded-full p-0 ${
                  isSelected ? "bg-black hover:bg-gray-800" : "bg-black hover:bg-gray-800"
                }`}
              >
                {index + 1}
              </Button>
            ...
            <div
              ...
              className={`flex-1 ${isExpanded ? "pb-[50px]" : ""} ${isSelected ? "bg-gray-50 p-2 rounded-lg" : ""}`}
              onClick={() => handleDayClick(index)}
            >
              <h3 className="mb-2 mt-2 text-lg font-medium text-black">{day.name}</h3>
