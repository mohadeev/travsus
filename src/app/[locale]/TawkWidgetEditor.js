...
export default function TawkWidgetEditor() {
	...
	useEffect(() => {
		setWidgets(tawkWidgets) // load initial values from JSON
	}, [])
	...
	const handleSave = async () => {
		try {
			// Save via an API route (Next.js does not allow writing JSON directly from client)
			const response = await fetch('/api/save-tawk-widgets', {
				method: 'POST',
