...
export default function PlanTripPage() {
	...
	const [suggestedCoordinates, setSuggestedCoordinates] = useState<
	...
	const addCity = () => {
		...
		// Remove the added city from suggestions
		setSuggestedCities(
	...
	const suggestCities = async () => {
		...
		if (!formData.duration) {
			setError('Please select a duration first')
			return
		}
		...
		try {
			...
			if (data.success && data.cities && data.cities.length > 0) {
				// Store current suggestions as previous before adding new ones
				setPreviousSuggestions([...previousSuggestions, ...suggestedCities])
				...
				setSuggestionError(
					data.message ||
						'No city suggestions available. Please try again or enter cities manually.',
				)
			}
		} catch (error) {
			console.error('Error suggesting cities:', error)
			setSuggestionError(
				'Failed to get city suggestions. Please try again or enter cities manually.',
			)
		} finally {
	...
	const nextStep = () => {
		switch (step) {
			...
			case 'duration':
				...
				// Reset suggestions when entering the cities selection step
				clearSuggestions()
				break
	...
	const getStepTitle = () => {
		switch (step) {
			...
			case 'tripType':
				return 'Trip Type'
			case 'itinerary':
				return itinerary?.title || 'Your Itinerary'
			case 'payment':
				return 'Confirm Your Trip Plan'
			case 'confirmation':
				return 'Your Trip is Reserved!'
			default:
				return 'Plan Your Trip with AI'
		}
	}
	...
	const getStepDescription = () => {
		switch (step) {
			case 'country':
				return 'Select the country you want to visit'
			case 'cities-selection':
				return 'Add the cities you want to visit in order. This will be your travel route.'
			case 'duration':
				return 'How many days would you like to travel?'
			case 'travelers':
				return 'How many people will be traveling?'
			case 'accommodation':
				return 'What type of accommodation would you prefer?'
			case 'groupType':
				return 'Would you prefer a private tour or to join a small group?'
			case 'tripType':
				return 'What kind of experience are you looking for?'
			case 'itinerary':
				return itinerary?.subtitle || 'Review your personalized itinerary'
			case 'payment':
				return 'To secure this itinerary, we require a €50 deposit. This amount will be credited toward your final booking.'
			case 'confirmation':
				return `We've received your €50 deposit. Our travel expert will contact you at ${formData.email} within 24 hours to finalize your booking details.`
			default:
				return 'Let our AI help you create the perfect travel experience'
		}
	}

	return (
		<div className="min-h-screen bg-[#F5F5F7] pb-20">
			<header className="w-full bg-black p-4 text-white md:p-6">
				<div className="container mx-auto">
					<div className="flex items-center">
						...
						<div>
							<h1 className="text-2xl font-bold">Plan Your Trip with AI</h1>
			...
			<div className="container mx-auto mt-8 px-4">
							...
							<div key={i} className="flex-1 text-center text-sm text-gray-500">
								{i ===
									...
									<span className="font-medium text-black">
										{s.split('-')[0].charAt(0).toUpperCase() +
											s.split('-')[0].slice(1)}
									</span>
								)}
							</div>
				...
				{/* Main content */}
				<div className="mx-auto max-w-4xl rounded-lg bg-white p-6 shadow-md md:p-8">
					<div className="mb-6">
					...
					<div className="mx-auto max-w-3xl">
						...
						{step === 'cities-selection' && (
							<div className="space-y-6">
								<div className="space-y-3">
									...
									<div className="flex items-center justify-between">
										<p className="text-sm text-gray-500">
											{selectedCities.length === 0
												...
												: `Planning a ${formData.duration}-day trip from ${selectedCities[0].name}`}
										</p>
									...
									{/* Map Component */}
									{showMap && formData.country && (
										<div className="mt-4">
											<TripMap
												selectedCities={selectedCities}
												suggestedCities={suggestedCities}
												suggestedCoordinates={suggestedCoordinates}
												onSuggestedCityClick={handleSuggestedCityClick}
												country={formData.country}
											/>
										</div>
									)}
										...
										<div className="mt-4">
											<div className="mb-2 flex items-center justify-between">
												<p className="text-sm font-medium">
													{selectedCities.length > 0
														? `Recommended cities for your trip:`
														: `Suggested cities in ${formData.country}:`}
												</p>
												<span className="text-xs text-gray-500">
													{suggestedCities.length} cities
												</span>
											</div>
									...
									{selectedCities.length > 0 && (
										<div className="mt-6">
											<p className="mb-3 font-medium">Your travel route:</p>
													...
													>
														...
														<div className="flex items-center gap-1">
															...
															<Button
																variant="ghost"
																size="icon"
																className="h-8 w-8 bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-700"
						...
						{step === 'accommodation' && (
							<div className="space-y-6">
								<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
									<div
										className={`cursor-pointer rounded-md border p-5 hover:bg-gray-50 ${
									...
									>
										...
										<p className="mt-2 pl-7 text-sm text-gray-600">
						...
						{step === 'itinerary' && itinerary && (
							<div className="space-y-6">
								<div className="mt-2 flex flex-col text-sm text-gray-600 md:flex-row md:justify-between">
						...
						{step === 'confirmation' && (
							<div className="space-y-6 text-center">
								...
								<div className="mt-8 rounded-lg bg-gray-100 p-5 text-left">
					...
					{step !== 'confirmation' && (
						<div className="mt-8 flex justify-between border-t border-gray-200 pt-6">
							...
							<Button
								...
								className={cn(
									'bg-black px-6 text-white hover:bg-gray-800',
