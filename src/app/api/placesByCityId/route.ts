...
export async function GET(request: Request) {
  try {
    ...
    if (places.length === 0 || places.every((p) => p.content.translations.length === 0)) {
    ...
    const placesData = places.map((place) => {
      ...
      const nameTranslation = place.content.translations.find((t) => t.type === "name")
      const descriptionTranslation = place.content.translations.find((t) => t.type === "description")
    ...
    return NextResponse.json({
      ...
      languageAvailable: city.content.translations.length > 0 && places.some((p) => p.content.translations.length > 0),
    })
  } catch (error: any) {
    console.error("❌ Error fetching places by city ID:", error?.message || error)
    return NextResponse.json(
      {
