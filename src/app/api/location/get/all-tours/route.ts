...
async function getOptimizedToursWithTranslations(language: string, page: number, limit: number) {
  ...
  if (cityIds.size > 0) {
    try {
      ...
      cities.forEach((city) => {
        const cityTranslation = city.content?.translations?.find((t) => t.language === language)
        const cityName =
          cityTranslation?.text ||
          city.content?.translations?.find((t) => t.language === "en-US")?.text ||
          "Unknown City"

        let countryInfo = null
        if (city.country) {
          const countryTranslation = city.country.content?.translations?.find((t) => t.language === language)
          countryInfo = {
            name:
              countryTranslation?.text ||
              city.country.content?.translations?.find((t) => t.language === "en-US")?.text ||
              "Unknown Country",
...
export async function GET(request: NextRequest) {
  try {
    ...
    const paginatedToursWithContent = await getOptimizedToursWithTranslations(language, page, limit)
