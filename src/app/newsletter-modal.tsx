...
const messageMap = {
  EMAIL_REQUIRED: "📭 Oops! It looks like you forgot to enter an email address. Let's try that again!",
  INVALID_EMAIL:
  ...
  TYPES_REQUIRED: "🎯 Almost there! Don't forget to pick at least one exciting newsletter type!",
  ALREADY_SUBSCRIBED:
...
export default function NewsletterModal() {
  ...
  const [selectedTypes, setSelectedTypes] = useState<string[]>(newsletterTypes.map((type) => type.id))
  ...
  useEffect(() => {
    ...
    const hasClosedModal = localStorage.getItem("hasClosedNewsletterModal") === "true"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(messageMap[data.code as keyof typeof messageMap] || messageMap.SUBSCRIPTION_SUCCESS)
        setIsError(false)
        setEmail("")
        ...
        setMessage(messageMap[data.code as keyof typeof messageMap] || messageMap.SERVER_ERROR)
        setIsError(true)
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="relative flex h-auto max-h-[90vh] w-[90%] max-w-4xl overflow-hidden rounded-lg bg-white">
        ...
        <button
          onClick={() => {
            setIsOpen(false)
            localStorage.setItem("hasClosedNewsletterModal", "true")
          }}
          ...
          <div className="flex h-[500px] max-h-[calc(90vh-140px)] flex-col justify-between">
            ...
            >
              <div className="min-h-[400px]">
                <h1 className="text-4xl font-extrabold text-black sm:text-5xl">SIGN UP TODAY</h1>
                ...
                <form id="newsletter-form" onSubmit={handleSubmit} className="mt-8">
                  {/* Email input first */}
                  ...
                  <div className="relative mt-4" ref={dropdownRef}>
                    <button
                      ...
                      disabled={isLoading}
                    >
                      <span>Select newsletter types ({selectedTypes.length} selected)</span>
                      <svg
                        className="h-5 w-5 text-gray-400"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 20 20"
