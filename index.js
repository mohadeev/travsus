...
const CONFIG = {
	...
	OPENAI_API_KEY:
		'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',
	OPENAI_MODEL: 'gpt-4',
...
async function initializeLanguageFiles() {
	await fs.mkdir(CONFIG.MESSAGES_DIR, { recursive: true })

	// Create empty files for all supported languages if they don't exist
	for (const langCode of Object.keys(CONFIG.LANGUAGES)) {
		...
		try {
			...
			console.log(`📄 Created empty translation file: ${filename}`)
		}
	}
}
...
async function translateMessages(englishMessages, targetLang) {
	...
	const targetLanguageName = CONFIG.LANGUAGES[targetLang]

	const prompt = `
# STRICT TRANSLATION INSTRUCTIONS

Translate these UI strings from English to ${targetLanguageName} (${targetLang}):
...
async function updateTranslationFiles(lang, messages) {
	// Convert to filename format (en-US → en_US.json)
	...
	try {
		...
		// File doesn't exist yet (shouldn't happen as we initialized them)
...
async function main() {
	try {
		console.log('🚀 STARTING MULTI-LANGUAGE I18N AUTOMATION')
				...
				for (const langCode of Object.keys(CONFIG.LANGUAGES)) {
					try {
						...
						if (langCode === 'en-US') {
							...
							messages = await translateMessages(result.messages, langCode)
						}

						// Update translation file
						await updateTranslationFiles(langCode, messages)
					} catch (error) {
						console.error(
							`❌ TRANSLATION FAILED FOR ${langCode}:`,
							error.message,
						)
					}
				}
		...
		console.log('\n🎉 INTERNATIONALIZATION COMPLETE FOR ALL LANGUAGES!')
	} catch (error) {
